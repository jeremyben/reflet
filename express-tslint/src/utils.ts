 ...
 * As a viable alternative, we simply use the dynamic 'import()' type (requires TypeSript 2.9).
 */
...
export function extendsType(type: ts.Type, baseType: ts.Type): boolean {
	return type.symbol === baseType.symbol
		? true
		: type.isIntersection()
		? type.types.some((t) => extendsType(t, baseType))
		: type.isUnion()
		? type.types.every((t) => extendsType(t, baseType))
		: (type.getBaseTypes() || []).some((t) => extendsType(t, baseType))
}
